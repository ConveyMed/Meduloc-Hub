...
-- Authenticated users can read all preferences (needed for push notification targeting)
CREATE POLICY "Authenticated users can read all preferences for notifications"
  ON user_notification_preferences
  FOR SELECT
  TO authenticated
  USING (true);
