...
const EMAIL = process.env.TEST_EMAIL || 'Chase@appcatalyst.org';
const PASSWORD = process.env.TEST_PASSWORD || 'Pri123456!';
const OUTPUT_DIR = path.join(__dirname, '..', 'screenshots');

// App Store required device sizes (iOS)
// App Store accepts: 1284x2778 (6.5" iPhone) and 2048x2732 (iPad)
const IOS_DEVICES = [
  {
    name: 'iPhone-6.5',
...
// Google Play required device sizes (Android)
...
// Select devices based on CLI arg: --android, --ios, or both (default)
const args = process.argv.slice(2);
const runAndroid = args.includes('--android') || (!args.includes('--ios'));
const runIOS = args.includes('--ios') || (!args.includes('--android'));
const DEVICES = [
  ...(runIOS ? IOS_DEVICES : []),
  ...(runAndroid ? ANDROID_DEVICES : []),
];
...
async function runForDevice(device) {
  ...
  const context = await browser.newContext({
    ...
    isMobile: device.name.startsWith('iPhone') || device.name === 'Android-Phone',
    hasTouch: true,
  });
    ...
    try {
      if (await clickNavButton(page, 'AI Agent')) {
        ...
        if (selectedProduct) {
          ...
          const chatInput = page.locator('textarea:not([disabled])').last();
      ...
      console.log(`  (04-AIChat error, skipping: ${aiErr.message.substring(0, 80)})`);
      // Try to recover by navigating home
      await clickNavButton(page, 'Home').catch(() => {});
      await sleep(2000);
    }

    // ---- 05: Downloads ----
    // Make sure we're fully away from AI Chat first
    await clickNavButton(page, 'Home').catch(() => {});
    await sleep(2000);
